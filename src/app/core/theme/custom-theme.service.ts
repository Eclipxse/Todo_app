...
const STRAWBERRY_MILK_REF: CustomThemeRef = {
  kind: 'builtin',
  id: 'strawberry-milk',
};
 ...
 * this personalized build opens with Strawberry Milk on every platform.
 *
...
export const pickInitialActiveRef = (
  stored: string | null,
  _isAppleSilicon: boolean,
): CustomThemeRef => {
  if (stored) return parseRef(stored);
  return STRAWBERRY_MILK_REF;
};
...
export const BUILT_IN_THEMES: CustomTheme[] = [
  {
    ...
    kind: 'builtin',
    url: 'assets/themes/strawberry-milk.css',
    requiredMode: 'system',
  },
