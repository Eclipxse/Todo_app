...
describe('CustomThemeService', () => {
  ...
  it('exposes built-ins followed by user themes in the themes signal', () => {
    ...
    expect(all[0].id).toBe('strawberry-milk');
  ...
  describe('migrateLegacyCustomTheme', () => {
    ...
    it('is a no-op for unknown legacy ids (allowlist enforced)', async () => {
      ...
      expect(localStorage.getItem(LS.CUSTOM_THEME)).toBeNull();
      expect(service.activeRef()).toEqual({
        kind: 'builtin',
        id: 'strawberry-milk',
      });
    });
  ...
  describe('pickInitialActiveRef', () => {
    // The personalized first-run default is platform-independent.
    ...
    it('falls back to Strawberry Milk when stored value is missing', () => {
    ...
    it('falls back to default when a stored value is malformed', () => {
    ...
    it('picks Strawberry Milk on first run for Apple Silicon Macs', () => {
